...
export default function IndustrySelectionClient({ initialDifficultyLevels }: IndustrySelectionClientProps) {
    ...
    const loadCompetencies = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setIndustryCompetencies(data.industryCompetencies || {})
          setIndustryMetadata(data.industryMetadata || {})
        }

        // Fetch all competencies
        const compResponse = await fetch("/api/competencies")
        if (compResponse.ok) {
          const compData = await compResponse.json()
          setAllCompetencies(compData.competencies || [])
        }
      } catch (error) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.focusAreas && Array.isArray(data.focusAreas)) {
            ...
            const enabledFocusAreas = data.focusAreas.filter((area: FocusArea) => area.enabled)
  ...
  useEffect(() => {
    const loadSubcategories = async () => {
      if (selectedIndustry) {
        try {
          const response = await fetch(`/api/industry-settings?industry=${selectedIndustry}`)
          if (response.ok) {
            const data = await response.json()
            if (data.subcategory && data.industry) {
              // Extract subcategories from the response
              const industryData = await fetch("/api/competencies/industry")
              if (industryData.ok) {
                const industryResponse = await industryData.json()
                const industryMetadata = industryResponse.industryMetadata || {}
