...
export default function SimulationPreview() {
  ...
  useEffect(() => {
    const loadIndustryMetadata = async () => {
      try {
        const response = await fetch("/api/competencies/industry")
        if (response.ok) {
          const data = await response.json()
          setIndustryMetadata(data.industryMetadata || {})
        }
      } catch (error) {
  ...
  const handleContinueToSimulation = () => {
    ...
    // Route directly to the simulation session page with all necessary parameters
    router.push(`/simulation/session?simulationId=${simulationId}&industry=${industry}&subcategory=${subcategory}&difficulty=${difficulty}`)
  }
  ...
  useEffect(() => {
    const fetchIndustrySettings = async () => {
      try {
        const response = await fetch(`/api/industry-settings?industry=${industry}&subcategory=${subcategory}`);
        if (response.ok) {
          const data = await response.json();
          setAiRoleLabel(data.aiRoleLabel || undefined);
          setAiRoleDescription(data.aiRoleDescription || undefined);
          // Optionally store in sessionStorage for session page
          if (data.aiRoleLabel) sessionStorage.setItem("aiRoleLabel", data.aiRoleLabel);
          if (data.aiRoleDescription) sessionStorage.setItem("aiRoleDescription", data.aiRoleDescription);
        }
      } catch (error) {
