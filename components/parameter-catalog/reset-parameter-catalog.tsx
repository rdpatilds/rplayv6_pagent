...
export function ResetParameterCatalog({ onSuccess }: ResetParameterCatalogProps) {
  ...
  const handleReset = async () => {
    try {
      setLoading(true)
      const response = await fetch("/api/parameter-catalog/reset", {
        method: "POST",
      })

      if (!response.ok) {
        throw new Error("Failed to reset parameter catalog")
      }
      ...
      toast({
        title: "Error",
        description: "Failed to reset parameter catalog. Please try again.",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
