...
export function GuardrailsParametersManager() {
  const { toast } = useToast()
  const [selectedCategory, setSelectedCategory] = useState<string | undefined>(undefined)
  const { data: categories, error: categoriesError, isLoading: categoriesLoading, mutate: mutateCategories } = useParameterCategories({ type: "guardrail" })
  ...
  const [newParameter, setNewParameter] = useState({ name: "", description: "", examples: "" })
  ...
  const handleAddParameter = async () => {
    try {
      ...
      setNewParameter({ name: "", description: "", examples: "" })
  ...
  if (!categories?.length) {
    return (
      <Alert>
        <AlertDescription>No guardrail parameter categories found. Please create a category first.</AlertDescription>
      </Alert>
    )
  }
